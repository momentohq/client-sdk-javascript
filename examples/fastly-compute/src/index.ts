...
async function handleRequest(event: FetchEvent) {
    ...
    try {
      ...
      const setResp = await momento.set(cacheName, key, value, 10);
      ...
      const deleteResp = await momento.delete(cacheName, key);
...
class MomentoFetcher {
  ...
  private readonly backend: string;
    ...
    if (resp.status >= 200 && resp.status < 300) {
      ...
      throw new Error(`failed to retrieve item from cache ${cacheName}. Message: ${resp.statusText} | Status: ${resp.status}`);
    }
	}
	...
	async set(cacheName: string, key: string, value: string, ttl_seconds: number = 30) {
      ...
      backend: this.backend
		...
		if (resp.status >= 200 && resp.status < 300) {
			...
			throw new Error(`failed to set item into cache ${cacheName}. Message: ${resp.statusText} | Status: ${resp.status}`);
		}
	...
	async delete(cacheName: string, key: string) {
      ...
      backend: this.backend
		...
		if (resp.status >= 200 && resp.status < 300) {
			...
			throw new Error(`failed to delete ${key} from cache ${cacheName}. Message: ${resp.statusText} | Status: ${resp.status}`);
		}
