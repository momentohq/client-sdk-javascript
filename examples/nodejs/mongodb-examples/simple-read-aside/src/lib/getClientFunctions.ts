...
/* A function that gets the Momento API key and endpoint stored in AWS Secrets Manager.
The secrets were stored as a plaintext format in Secrets Manager to avoid parsing JSON.

You don't have to store the Momento API key or endpoint in something like AWS Secrets Manager,
but it is best practice. You could pass them in from environment variables.
...
export default async function CreateCacheClient(
  ...
  endpointSecretName: string = "Momento_Endpoint",
): Promise<CacheClient> {
  // Call the GetSecret function to get a Momento API key from AWS Secrets Manager.
  const apiKey: string = await GetSecret(apiKeySecretName);

  // Call the GetSecret function to get a Momento endpoint from AWS Secrets Manager.
  const endpoint: string = await GetSecret(endpointSecretName);

  // Get a new cache connection with the API key and set a default TTL for the connection.
  return await CacheClient.create({
    configuration: Configurations.Laptop.latest(),
    credentialProvider: CredentialProvider.fromApiKeyV2({ apiKey: apiKey, endpoint: endpoint }),
