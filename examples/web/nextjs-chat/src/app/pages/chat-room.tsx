...
export default function ChatRoom(props: Props) {
    ...
    return (
        <div className={"flex flex-col p-6 h-full justify-between bg-slate-200"}>
            <div className={'flex justify-between items-center p-1 border-b-2 border-slate-300 mb-4'}>
                <div>
                    Welcome to the <span className={'italic'}>{props.topicName}</span> chat room
                </div>
                <button onClick={props.onLeave} className={'bg-red-600 text-white rounded-xl p-1'}>Leave</button>
            </div>
            <div className={"h-full p-1 overflow-auto"}>{chats.map(chat => {
                ...
                switch (chat.event) {
                    case EventTypes.MESSAGE:
                        ...
                        return <div className={'break-words'} key={`${chat.timestamp}-${chat.username}`}><span className={'text-red-500'}>{timestampWithUsername}</span> {chat.text}</div>
                    case EventTypes.USER_JOINED:
                        const timestamp = `[${hours}:${minutes}]`;
                        return <div key={`${chat.timestamp}-${chat.username}`} className={'text-green-500 italic'}>{timestamp} user joined: {chat.username}</div>
                }
            })}
                <div ref={messagesEndRef} />
            </div>
            <div className={'w-full flex '}>
                <input placeholder={'chat'} onKeyDown={onEnterClicked} className={"border-2 rounded-2xl p-2 w-full"} value={textInput} onChange={(e) => setTextInput(e.target.value)}/>
                <div className={'w-4'} />
                <button onClick={onSendMessage} disabled={!textInput} className={'bg-blue-400 rounded-2xl p-2 hover:brightness-75 disabled:bg-slate-400 disabled:brightness-75 disabled:text-white'}>Send</button>
            </div>
        </div>
